...
fn create_pow_proof(
    ...
    let pow_proof = prover.prove(env, POW_PROGRAM_ELF).unwrap();
...
fn create_stone_object(prover: &impl risc0_zkvm::Prover) -> ObjectJson {
    let (obj, obj_hash) = mine_object(STONE_BLUEPRINT, STONE_MINING_MAX, vec![]);
    println!("Mined stone: seed={}, hash={}", obj.seed, obj_hash);

    let (pow_out, pow_proof) = create_pow_proof(prover, 3, obj_hash);
    ...
    let stone_proof = prover.prove(env, STONE_PROGRAM_ELF).unwrap();
...
fn create_axe_object(
    ...
    let axe_proof = prover.prove(env, AXE_PROGRAM_ELF).unwrap();
...
fn main() {
    ...
    for i in 1..=num_stones {
        println!("\n=== Creating Stone {} ===", i);
        let object = create_stone_object(&prover);
